		...
		case ELF_SHT_REL: {
			...
			for (uint i=0; i<relnum; i++) {
				...
				if (elf32) {
					...
					tt = tag_make_edit_qword(tt, tt_end, h+i*rel_size+offsetof(ELF_REL64, r_offset), endianness);
				}
				...
				if (elf32) {
					...
					tt = tag_make_edit_dword(tt, tt_end, h+i*rel_size+offsetof(ELF_REL64, r_sym), endianness);
				}
				tt += ht_snprintf(tt, tt_end, "   ");
				...
				if (elf32) {
					...
					tt = tag_make_edit_dword(tt, tt_end, h+i*rel_size+offsetof(ELF_REL64, r_type), endianness);
				}
				tt += ht_snprintf(tt, tt_end, "(");
				/* type */
				if (elf32) {
					...
					tt = tag_make_desc_dword(tt, tt_end, h+i*rel_size+offsetof(ELF_REL64, r_type), ATOM_ELF_R_X86_64_TYPE, endianness);
				}
				tt += ht_snprintf(tt, tt_end, ")");
				m->add_mask(t);
			}
		...
		case ELF_SHT_RELA: {
			...
			for (uint i=0; i<relnum; i++) {
				...
				if (elf32) {
					...
					tt = tag_make_edit_qword(tt, tt_end, h+i*rela_size+offsetof(ELF_RELA64, r_offset), endianness);
				}
				tt += ht_snprintf(tt, tt_end, " ");
				/* symbol (table idx) */
				if (elf32) {
					...
					tt = tag_make_edit_dword(tt, tt_end, h+i*rela_size+offsetof(ELF_RELA64, r_sym), endianness);
				}
				tt += ht_snprintf(tt, tt_end, "   ");
				/* addend */
				if (elf32) {
					...
					tt = tag_make_edit_qword(tt, tt_end, h+i*rela_size+offsetof(ELF_RELA64, r_addend), endianness);
				}
				tt += ht_snprintf(tt, tt_end, " ");
				/* type */
				if (elf32) {
					...
					tt = tag_make_edit_dword(tt, tt_end, h+i*rela_size+offsetof(ELF_RELA64, r_type), endianness);
				}
				tt += ht_snprintf(tt, tt_end, "(");
				/* type */
				if (elf32) {
					...
					tt = tag_make_desc_dword(tt, tt_end, h+i*rela_size+offsetof(ELF_RELA64, r_type), ATOM_ELF_R_X86_64_TYPE, endianness);
				}
				tt += ht_snprintf(tt, tt_end, ")");
				m->add_mask(t);
			}
